...
from .utils import _mstar_, makeHealpixMap
...
from .utils import read_hpix_mosaicFootprint, make_tile_cat


def store_wazp_confs(workdir, param_cfg, param_data):
    with open(os.path.join(workdir, "config", "wazp.cfg"), "w") as outfile:
        json.dump(param_cfg, outfile)
    config = os.path.join(workdir, "config", "wazp.cfg")
    with open(os.path.join(workdir, "config", "data.cfg"), "w") as outfile:
...
def compute_zpslices(zp_metrics, detection_cfg, zs_targ, workdir):
    output = os.path.join(workdir, detection_cfg["zpslices_filename"])

    zpmin, zpmax = np.float64(zp_metrics["zpmin"]), np.float64(zp_metrics["zpmax"])
    ...
    zsl_min = zsl - nsig * sigma_dz0(zp_metrics, zsl) * (1.0 + zsl)
    zsl_max = zsl + nsig * sigma_dz0(zp_metrics, zsl) * (1.0 + zsl)
    sig_dz = sigma_dz0(zp_metrics, zsl) * (1.0 + zsl)
    nsl = len(zsl)
...
def zp_weight_fct(zsl, zsl_min, zsl_max, x):
    sig = (zsl_max - zsl_min) / 4.0
    if x < zsl - sig:
        weight = (1.0 / sig) * x + 2.0 - (1.0 / sig) * zsl
    if x > zsl + sig:
        weight = -(1.0 / sig) * x + 2.0 + (1.0 / sig) * zsl
...
def bkg_from_hpx_counts(dat_footprint, footprint, ra, dec, weights):
    ...
    id_count0 = np.isin(fhpx, ghpx_kept_unique, assume_unique=True, invert=True)
    fhpx_count0 = fhpx[id_count0]
    counts0 = np.zeros(len(fhpx_count0))
    counts = makeHealpixMap(ra[idkept], dec[idkept], weights[idkept], Nside, nest)
    all_counts = np.concatenate((counts[counts > 0], counts0))
    bkg_arcmin2 = np.mean(all_counts) / (3600.0 * hp.nside2pixarea(Nside, degrees=True))
    return bkg_arcmin2
...
def bkg_tile_slice(
    ...
    ra, dec, weight = select_galaxies_in_slice(
        dat_galcat, galcat, detection_cfg, zpslices, mstar_file, dmag_faint, weight_mode
    )
    conv_factor = cosmo.angular_diameter_distance(zpslices["zsl"])
    area_mpc2 = (np.pi * conv_factor.value / (60.0 * 180.0)) ** 2
    bkg_arcmin2 = bkg_from_hpx_counts(dat_footprint, footprint, ra, dec, weight)
...
def bkg_tile(
    ...
    data_bkg = np.zeros(
        nsl, dtype={"names": ("zsl", "bkg_arcmin2", "bkg_mpc2"), "formats": ("f4", "f4", "f4")}
    )
...
def create_wcs_at_z(detection_cfg, tile_specs, z, cosmo_params):
    ...
    pix_mpc = 1.0 / float(detection_cfg["resolution"])
...
def vmap_from_hpx(dat_hpx, footprint, tile_specs, detection_cfg, cosmo_params, zsl):
    Nside, nest = footprint["Nside"], footprint["nest"]
    hpx = dat_hpx[footprint["key_pixel"]]

    # build vmap image header
    wvmap, nxy = create_wcs_at_z(detection_cfg, tile_specs, zsl, cosmo_params)
    ...
    ra_map0, dec_map0 = (
        wvmap.all_pix2world(xvr0 + 0.5, yvr0 + 0.5, 1)[0],
    ...
    vmap[xvr0[np.isin(hpx_map, hpx)], yvr0[np.isin(hpx_map, hpx)]] = 1.0
    return vmap
...
def map_detlum_weight(mag, mstar, detection_cfg):
    weight = np.ones(len(mag))
    if detection_cfg["map_lum_weight_mode"]:
        weight = detlum_weight(mag, mstar, detection_cfg["lum_weight_map_power"])
    return weight
...
def map_lum_weight(mag, mstar, detection_cfg):
    weight = np.ones(len(mag))
    if detection_cfg["map_lum_weight_mode"]:
        weight = lum_weight(mag, mstar, detection_cfg["lum_weight_map_power"])
    return weight
...
def select_galaxies_in_slice(
    dat_galcat, galcat, detection_cfg, zpslices, mstar_file, dmag_faint, weight_mode
):
    ra, dec = dat_galcat[galcat["keys"]["key_ra"]], dat_galcat[galcat["keys"]["key_dec"]]
    ...
    if weight_mode == "zplum":
        weight_map = zp_weight(zpslices["zsl"], zsl_min, zsl_max, zp_map) * map_lum_weight(
            mag_map, mstar, detection_cfg
        )
    if weight_mode == "zp":
...
def select_zps_cylinder(dcyl, dgal, galcat, zpslices, mstar_file, cosmo, wazp_specs):
    ...
    nsig = 3
    sig_dz = (zpslices["zsl_max"] - zpslices["zsl_min"]) / 4.0
    _zsl_min, _zsl_max = zpslices["zsl_min"], zpslices["zsl_max"]
    zsl = zpslices["zsl"]
    ...
    cond_disc = cond_in_disc(
        rag[cond],
        decg[cond],
        dgal["hpix_tmp"][cond],
        wazp_specs["Nside_tmp"],
        wazp_specs["nest_tmp"],
        dcyl["ra"],
        dcyl["dec"],
        rad_deg,
...
def select_cells_from_footprint(Nside_cell, data_fp, footprint):
    ...
    pix_out, frac_out = hpx_degrade(
        hpix_map, frac_map, footprint["Nside"], footprint["nest"], Nside_cell, footprint["nest"]
    )
...
def bkg_global_survey(galcat, footprint, tiling, cosmo_params, mstar_file, detection_cfg, workdir):
    # relevant paths and files
    tiles_filename = os.path.join(workdir, tiling["rpath"], tiling["tiles_filename"])
    zpslices_filename = os.path.join(workdir, detection_cfg["zpslices_filename"])
    os.path.join(workdir, tiling["rpath"], tiling["sky_partition_npy"])
    gbkg_filename = os.path.join(workdir, "gbkg", detection_cfg["gbkg_filename"])
    ...
    hpix_core_lists = np.load(
        os.path.join(workdir, tiling["rpath"], tiling["sky_partition_npy"]), allow_pickle=True
    )

    area_min = detection_cfg["gbkg_area"]
    # select galcat and associated footprint to reach some
    # minimum area in the survey
    flat(H0=cosmo_params["H"], Om0=cosmo_params["omega_M_0"])
    tiles = read_FitsCat(tiles_filename)
    read_FitsCat(zpslices_filename)
...
def bkg_global(
    ...
    _nbar_rich, nbar_snr = np.zeros(len(zpslices)), np.zeros(len(zpslices))
    ...
    _lbar_rich, _lbar_snr = np.zeros(len(zpslices)), np.zeros(len(zpslices))

    # effective used area
    area_eff = float(len(data_fp)) * hp.nside2pixarea(footprint["Nside"], degrees=True)

    # loop over zsl and select resolution
    jinf, jsup = np.zeros(len(zpslices)).astype(int), np.zeros(len(zpslices)).astype(int)
    ...
    for i in range(0, len(zpslices)):
        ...
        if ksi2_inf > 0.0 and ksi2_sup > 0:
            beta_snr[i] = (
                2.0
                * np.log(ksi2_sup / ksi2_inf)
                / np.log(hp.nside2pixarea(2 ** jsup[i]) / hp.nside2pixarea(2 ** jinf[i]))
            )
            lg_ksi2 = np.log(ksi2_sup) + np.log(ksi2_inf / ksi2_sup) * np.log(
                cell_area[i] / hp.nside2pixarea(2 ** jsup[i], degrees=True)
            ) / np.log(hp.nside2pixarea(2 ** jinf[i]) / hp.nside2pixarea(2 ** jsup[i]))
        else:
            beta_snr[i] = beta_default
            lg_ksi2 = max(ksi2_inf, ksi2_sup)

        nbar_snr[i] = nbar_sup * cell_area[i] / hp.nside2pixarea(2 ** jsup[i], degrees=True)
...
def compute_catimage(
    ra_map, dec_map, weight_map, zpslices, detection_cfg, tile_specs, cosmo_params
):
...
def compute_filled_catimage(
    ...
    nlist = hp.get_all_neighbours(
        footprint["Nside"], data_footprint[footprint["key_pixel"]], None, footprint["nest"]
    )
    ...
    shpix_filled = np.unique(
        hp.ang2pix(footprint["Nside"] * 2, ra_map, dec_map, footprint["nest"], lonlat=True)
    )
    hpix_filled = np.unique(
        hp.ang2pix(footprint["Nside"], ra_map, dec_map, footprint["nest"], lonlat=True)
    )

    edge_pixels[np.isin(edge_pixels, hpix_filled, invert=True)]
    ...
    hpx_ran1 = hp.ang2pix(footprint["Nside"], ra_ran, dec_ran, footprint["nest"], lonlat=True)
    ra_ranf1 = ra_ran[np.isin(hpx_ran1, data_footprint[footprint["key_pixel"]], invert=True)]
    dec_ranf1 = dec_ran[np.isin(hpx_ran1, data_footprint[footprint["key_pixel"]], invert=True)]
    hpx_ran2 = hp.ang2pix(footprint["Nside"] * 2, ra_ran, dec_ran, footprint["nest"], lonlat=True)
...
def wmap2peaks(wmap, wazp_specs, tile_specs, zsl, cosmo_params):
    ...
    len(local_maxi)  # nr of peaks in the filled_map
    iobj0, jobj0 = local_maxi[:, 0] + 1.0, local_maxi[:, 1] + 1.0  # pixel coord of the peaks
...
def filter_peaks(admin, tile, zsl, cosmo_params, resolution, ra0, dec0, ip0, jp0):
    if not admin["target_mode"]:
        cosmo = flat(H0=cosmo_params["H"], Om0=cosmo_params["omega_M_0"])
        err_mpc = 2.0 / float(resolution)  # +/- 2 pixels around the tile
        ...
        ghpx = hp.ang2pix(tile["Nside"], ra0, dec0, tile["nest"], lonlat=True)
        ghpx1 = hp.ang2pix(tile["Nside"], ra0 - dx, dec0 - dy, tile["nest"], lonlat=True)
        ghpx2 = hp.ang2pix(tile["Nside"], ra0 - dx, dec0 + dy, tile["nest"], lonlat=True)
        ghpx3 = hp.ang2pix(tile["Nside"], ra0 + dx, dec0 + dy, tile["nest"], lonlat=True)
        ghpx4 = hp.ang2pix(tile["Nside"], ra0 + dx, dec0 - dy, tile["nest"], lonlat=True)
        cond_filter = (
            (np.isin(ghpx, tile["hpix_core"]))
        ...
        cond_filter = (
            np.degrees(dist_ang(ra0, dec0, tile["ra"], tile["dec"])) <= tile["radius_filter_deg"]
        )
...
def coverfrac_disc(ra, dec, data_fp, footprint, radius_deg):
    ...
    for i in range(0, len(ra)):
        hpx_in_ann, frac_in_ann, area_deg2, coverfrac[i] = hpx_in_annulus(
            ra[i], dec[i], 0.0, radius_deg, data_fp, footprint, False
        )
    return coverfrac
...
def init_peaks_table(ra_peaks, dec_peaks, iobj, jobj, coverfrac, wradius, zsl):
    ...
    data_peaks = np.zeros(
        (len(ra_peaks)),
        dtype={
            "names": ("id", "ra", "dec", "iobj", "jobj", "z", "coverfrac", "wradius_mpc"),
            "formats": ("i8", "f8", "f8", "f4", "f4", "f4", "f4", "f4"),
        },
    )
...
def compute_flux_aper(rap, decp, hpix, weight, aper, Nside, nest):
    ...
    pixelized_area = float(len(pixels_in_disc)) * hp.nside2pixarea(Nside, degrees=True)
    return Nraw * area_ann_deg2(0.0, aper) / pixelized_area


def compute_flux_aper_vec(
    rap,
    decp,
    aper,
    dat_galcat,
    ...
    for i in range(0, len(rap)):
        Naper[i] = compute_flux_aper(rap[i], decp[i], hpix, weight, aper, Nside, nest)
...
def add_peaks_attributes(
    ...
    sig_N = (gbkg["nbar_snr"] * (1.0 + gbkg["ksi2"])) ** 0.5
    sig_L = (gbkg["lbar_snr"] * (1.0 + gbkg["ksi2"])) ** 0.5

    3600.0 * area_ann_deg2(0.0, aper_deg)  # arcmin2
...
def init_cylinders(keyrank, peak_ids, wazp_specs):
    ...
    data_cylinders = np.zeros(
        (ncyl),
        dtype={
            "names": ("id", "peak_id", "cyl_length", "cyl_isl_min", "cyl_isl_max", "cyl_isl_mode"),
...
def append_peaks_infos_to_cylinders(
    data_cylinders_init, peaks_list, zpslices_specs, ip_cyl, ra_cyl, dec_cyl, rank_cyl, snr_cyl
):
        ...
        if flag_min == 1:
            ...
            if isl == islmin:  # all peaks are new cylinders
                ...
                if len(peaks_list[isl - 1]) == 0:  # no detection in intermediate slice
                    ...
                    rank_cyl_new, snr_cyl_new = (
                        np.ones((np0, nsl)) * (-1),
                        np.ones((np0, nsl)) * (-1),
                    )
                    ra_cyl_new, dec_cyl_new = np.ones((np0, nsl)) * (-1), np.ones((np0, nsl)) * (-1)
                    ...
                    rank_cyl, snr_cyl = (
                        np.concatenate((rank_cyl, rank_cyl_new)),
                        np.concatenate((snr_cyl, snr_cyl_new)),
                    )
                    ra_cyl, dec_cyl = (
                        np.concatenate((ra_cyl, ra_cyl_new)),
                    ...
                    ip_cyl = key_cylinder(ip_cyl, id_1, np0, i0, i1, isl, nsl, type="int")
                    rank_cyl = key_cylinder(rank_cyl, rank_1, np0, i0, i1, isl, nsl, type="float")
                    snr_cyl = key_cylinder(snr_cyl, snr_1, np0, i0, i1, isl, nsl, type="float")
                    ra_cyl = key_cylinder(ra_cyl, ra_1, np0, i0, i1, isl, nsl, type="float")
                    dec_cyl = key_cylinder(dec_cyl, dec_1, np0, i0, i1, isl, nsl, type="float")

    if npeaks_all > 0:
        data_init = init_cylinders(rank_cyl, ip_cyl, wazp_specs)
        data_cylinders = append_peaks_infos_to_cylinders(
            data_init, peaks_list, zpslices_specs, ip_cyl, ra_cyl, dec_cyl, rank_cyl, snr_cyl
        )
...
def peak_fwhm(smoo, imax):
    smooo = smoo[imax : len(smoo) - 1] - smoo[imax] / 2.0
    idp = np.argwhere(smooo <= 0.0).T[0]
    if len(idp) > 0:
        idpp = np.amin(idp)
        idporg = idpp + imax
    else:
    ...
    smooob = smoo[0 : imax - 1] - smoo[imax] / 2.0
    idm = np.argwhere(smooob <= 0.0).T[0]
    if len(idm) > 0:
        idmorg = np.amax(idm)
    else:
        idmorg = 0
    delta = min(imax - idmorg, idporg - imax)
    z_precision_index = float(delta) / 4.0 - 1.0

    return z_precision_index


def plot_zhist_in_cylinder(
    dcyl,
    zin,
    wazp_specs,
    zpslices_specs,
    ...
    id_cyl = dcyl["id"]
    zsl = zpslices_specs["zsl"]
    sig_dz = (zpslices_specs["zsl_max"] - zpslices_specs["zsl_min"]) / 4.0
    zmin_z = zsl[dcyl["cyl_isl_min"]] - nsig * sig_dz[dcyl["cyl_isl_min"]]
    zmax_z = zsl[dcyl["cyl_isl_max"]] + nsig * sig_dz[dcyl["cyl_isl_max"]]
    ...
    area_cl_arcmin2 = 3600.0 * np.pi * rad_deg_faint**2
    tile_area_arcmin2 = 3600.0 * tile_specs["framed_eff_area_deg2"]
    magstar = _mstar_(mstar_file, z_cyl)
    maglim_faint = magstar + wazp_specs["dmag_zdet"]

    plt.clf()
    npts = int(10.0 * (zmax_z - zmin_z) / sig_dz[isl_mode]) + 1
    ...
    nw_bkg, bin_edges = np.histogram(
        zbkg, bins=npts, range=(zmin_z, zmax_z), weights=None, density=None
    )
    ...
    smoo_bkg = (
        convolve(nw_bkg, gauss_kernel, boundary="extend") * area_cl_arcmin2 / tile_area_arcmin2
    )
    smoo_faint_eff = smoo_faint - smoo_bkg
    smoo_faint_eff[smoo_faint_eff < 0.0] = 0.0
    imax_faint = np.argmax(smoo_faint)

    plt.axvline(x=zmin_z, color="black", linestyle="--", label="slices in cylinder")
    for ii in range(isl_min, isl_max + 1):
        plt.axvline(x=zsl[ii], color="black", linestyle="--")

    plt.axvline(x=z_cyl, color="black", linestyle="-", label="initial redshift")
    ...
    for ii in range(0, len(zin)):
        if ii == 0:
            plt.axvline(x=zin[ii], color="red", linestyle="-", label="refined redshift ")
        else:
            plt.axvline(x=zin[ii], color="red", linestyle="--", label="2ndary detection")
    plt.axis([zmin_z, zmax_z, 0.0, max(nw_faint) * 1.1])
    plt.xlabel("redshift")
    plt.ylabel("counts")
    plt.legend(loc=1)
    plt.grid(True)
...
def plot2(
    ...
    plt.axvline(
        x=zsl[isl_min], color="black", linestyle="--", label="slices in cylinder"
    )  # show all slices in cylinder
    for zz in zsl[isl_min : isl_max + 1]:
        plt.axvline(x=zz, color="black", linestyle="--")  # show all slices in cylinder
    plt.axvline(x=z_cyl, color="black", linestyle="-", label="initial redshift")
    for ii in range(0, len(zin)):
        if ii == 0:
            plt.axvline(x=zin[ii], color="red", linestyle="-", label="refined redshift ")
            ...
            plt.axvline(x=zin[ii], color="red", linestyle="--", label="2ndary detection")
    plt.axis([zmin_z, zmax_z, 0.0, max(contrast_faint) * 1.1])
    plt.xlabel("redshift")
    plt.ylabel("contrast")
    plt.legend(loc=1)
    plt.grid(True)
...
def z_local_maxima(dcyl, tile_specs, wazp_specs, zpslices_specs, ztest_faint, zbkg_faint, cosmo):
    gauss_kernel = Gaussian1DKernel(4)
    nsig = 3
    sig_dz = (zpslices_specs["zsl_max"] - zpslices_specs["zsl_min"]) / 4.0
    isl_mode = dcyl["cyl_isl_mode"]
    ...
    nw_faint, bins = np.histogram(
        ztest_faint, npts, range=(zmin_z, zmax_z), weights=None, density=None
    )
    nw_bkg, bins = np.histogram(
        zbkg_faint, npts, range=(zmin_z, zmax_z), weights=None, density=None
    )
    xx = (bins[0 : len(bins) - 1] + bins[1 : len(bins)]) / 2.0
    smoo_faint = convolve(nw_faint, gauss_kernel, boundary="extend")
    smoo_bkg = (
    ...
    # init peaks
    z_precision = -1.0 * np.ones(1)
    z_contrast = -1.0 * np.ones(1)
    ibest = isl_mode * np.ones(1).astype(int)
    zin = np.array([zsl[isl_mode]])
    ncl_in_cyl = 1

    # find all local maxima
    if np.amax(smoo_faint_eff) > 0.0:
        arr = (
            (np.r_[True, smoo_faint_eff[1:] > smoo_faint_eff[:-1]])
        ...
        if len(imax_insort) > 0:
            ...
            for ii in range(0, len(zin)):
                ...
                ibest[ii] = isl_min + np.argmin(np.absolute(zsl[isl_min : isl_max + 1] - zin[ii]))
...
def cylinders2clusters(
    ...
    # read zp specs
    (zpslices_specs["zsl_max"] - zpslices_specs["zsl_min"]) / 4.0
    zpslices_specs["zsl"]
    ...
    icyl = np.zeros(2 * ncyl).astype(int)  # max length assuming 2 cl/cyl
    ...
    for i in range(0, ncyl):
        ...
        ztest_faint, zbkg_faint = select_zps_cylinder(
            data_cyl[i], data_gal, galcat, zpslices_specs, mstar_file, cosmo, wazp_specs
        )
        ncl_in_cyl0, zin, z_precision0, z_contrast0, ibest = z_local_maxima(
            data_cyl[i], tile_specs, wazp_specs, zpslices_specs, ztest_faint, zbkg_faint, cosmo
        )
        # if (verbose >=2 and data_cyl[i]['snr']>10.) or
        # (verbose >=2 and z_precision0[0]==-1.):
        if verbose >= 2 and z_precision0[0] == -1.0:
...
def cl_duplicates_filtering(data_clusters0, wazp_specs, clcat, zpslices_specs, cosmo_params, mode):
    ...
    # zpmin / zpmax
    condzmax = data_clusters0[clcat["wazp"]["keys"]["key_zp"]] <= zpslices_specs["zsl_max"][::-1][0]
    condzmin = data_clusters0[clcat["wazp"]["keys"]["key_zp"]] >= zpslices_specs["zsl"][0]
    data_clusters_in = data_clusters0[condzmin & condzmax]
    ...
    for i in range(0, len(data_cl)):
        if flagdp[i] == 0:
            ...
            if mode == "tile":
                cond = (np.absolute(zcl[i] - zcl) < nsigdz * sig_dz[i]) & (clid[i] != clid)
            if mode == "survey":
                cond = (np.absolute(zcl[i] - zcl) < nsigdz * sig_dz[i]) & (tile_id[i] != tile_id)
            in_cone = cond_in_disc(
                ra[cond], dec[cond], clhpx[cond], Nside_tmp, nest_tmp, ra[i], dec[i], radius_deg
            )
...
def append_infos_to_clusters(target, data_clusters_init, cosmo_params):
    ...
    dist_arcmin = 60.0 * np.degrees(
        dist_ang(data_clusters_init["ra"], data_clusters_init["dec"], ra_target, dec_target)
    )
    conv_factor = cosmo.angular_diameter_distance(zcl)
    dist_mpc = np.radians(dist_arcmin / 60.0) * conv_factor.value

    data_extra = np.zeros(
        (len(zcl)), dtype={"names": ("dist_arcmin", "dist_mpc"), "formats": ("f8", "f8")}
    )
...
def wave_radius(wmap_data, ip, jp, detection_cfg):
    dwmap = ndi.distance_transform_edt((wmap_data > detection_cfg["wmap_thresh"]) * wmap_data)
...
def wazp_tile_slice(
    ...
    xycat_fitsname = os.path.join(
        paths["workdir_loc"], paths["detection"]["files"], "xycat_" + str(isl) + ".fits"
    )
    xycat_fi_fitsname = os.path.join(
        paths["workdir_loc"], paths["detection"]["files"], "xycat_fi_" + str(isl) + ".fits"
    )
    wmap_fitsname = os.path.join(
        paths["workdir_loc"], paths["detection"]["files"], "wmap_" + str(isl) + ".fits"
    )
    peaks_fitsname = os.path.join(
        paths["workdir_loc"], paths["detection"]["files"], "peaks_" + str(isl) + ".fits"
    )

    # select objects for computing density maps
    ra_map, dec_map, weight_map = select_galaxies_in_slice(
        dat_galcat, galcat, detection_cfg, zpslices, mstar_file, detection_cfg["dmag_det"], "detlum"
    )
    xycat = compute_catimage(
        ra_map, dec_map, weight_map, zpslices, detection_cfg, tile, cosmo_params
    )
    ...
    if not os.path.isfile(wmap_fitsname):
        map2fits(xycat_fi, detection_cfg, tile, zpslices["zsl"], cosmo_params, xycat_fi_fitsname)
    ...
    rap, decp, ip, jp = filter_peaks(
        ...
        zpslices["zsl"],
        cosmo_params,
        detection_cfg["resolution"],
        rap0,
        decp0,
        ip0,
        jp0,
    )  # to keep inner tile peaks
    wradius_mpc = wave_radius(wmap_data, ip.astype(int), jp.astype(int), detection_cfg)
    ...
    pcond = (coverfrac > 0.0) & (wradius_mpc > detection_cfg["radius_snr_mpc"])
    ...
    if verbose >= 1:
        ...
        print("..............peaks filtering wradius    in / out ", len(rap), len(rap[pcond]))
        print(
            "..............peaks filtering SNR        in / out ",
            len(data_peaks),
            len(data_peaks[data_peaks["snr"] >= np.float64(detection_cfg["snr_min"])]),
        )

        eff_area_mpc2 = tile["eff_area_deg2"] * (np.pi * conv_factor.value / 180.0) ** 2
        framed_eff_area_mpc2 = (
            tile["framed_eff_area_deg2"] * (np.pi * conv_factor.value / 180.0) ** 2
        )
        print(
            "         area mpc2 / deg2 ",
            np.round(eff_area_mpc2, 2),
            np.round(tile["eff_area_deg2"], 2),
        )
        print(
            "         galaxies density/arcmin2 /mpc2",
            np.round(float(len(ra_map)) / (tile["framed_eff_area_deg2"] * 3600.0), 3),
            np.round(float(len(ra_map)) / framed_eff_area_mpc2, 3),
        )
        print(
            "         clusters density/mpc2 ",
            np.round(
                float(len(data_peaks[data_peaks["snr"] >= detection_cfg["snr_min"]]))
                / eff_area_mpc2,
                3,
            ),
        )
    ...
    if verbose >= 2:
        map2fits(xycat, detection_cfg, tile, zpslices["zsl"], cosmo_params, xycat_fitsname)
...
def wazp_tile(
    ...
    if os.path.isfile(os.path.join(tile_dir, out_paths["detection"]["results"], "clusters.fits")):
        return
    ...
    if dat_galcat is None:
        data_gal_tile, data_fp_tile = make_tile_cat(
            tile_specs, galcat, footprint, maglim, detection_cfg
        )
        if verbose >= 2:
    ...
    zpslices = read_FitsCat(os.path.join(workdir, detection_cfg["zpslices_filename"]))
    gbkg = read_FitsCat(os.path.join(workdir, "gbkg", detection_cfg["gbkg_filename"]))
    ...
    if not os.path.isfile(
        os.path.join(out_paths["workdir_loc"], out_paths["detection"]["results"], "clusters0.npy")
    ):
        peaks_list = []
        npeaks_tot = 0
        ...
        if npeaks_tot > 0:
            ...
            data_cylinders = make_cylinders(peaks_list, zpslices, detection_cfg, cosmo_params)
            if verbose >= 1:
            ...
            if data_cylinders is not None:
                ...
                np.save(
                    os.path.join(
                        out_paths["workdir_loc"], out_paths["detection"]["results"], "clusters0.npy"
                    ),
                    data_clusters0,
                )
    ...
    if data_clusters0 is not None:
        data_clusters = cl_duplicates_filtering(
            data_clusters0, detection_cfg, clcat, zpslices, cosmo_params, "tile"
        )
    ...
    tile_info = np.zeros(
        1, dtype={"names": ("id", "eff_area_deg2", "Nclusters"), "formats": ("i8", "f8", "i8")}
    )
    ...
    tile_info.write(
        os.path.join(out_paths["workdir_loc"], out_paths["detection"]["results"], "tile_info.fits"),
        overwrite=True,
    )
    ...
    if data_clusters is not None:
        ...
        t.write(
            os.path.join(tile_dir, out_paths["detection"]["results"], "clusters.fits"),
            overwrite=True,
        )
    return
...
def tiles_with_clusters(out_paths, all_tiles):
    ...
    for it in range(0, len(all_tiles)):
        ...
        if os.path.isfile(
            os.path.join(tile_dir, out_paths["detection"]["results"], "tile_info.fits")
        ):
            if (
                read_FitsCat(
                    os.path.join(tile_dir, out_paths["detection"]["results"], "tile_info.fits")
...
def wmap_at_zcl(
    ...
    for i in range(0, len(zcl)):
        xycat_fitsname = os.path.join(workdir, path_outputs, "xycat_cl" + str(i) + ".fits")
        wmap_fitsname = os.path.join(workdir, path_outputs, "wmap_cl" + str(i) + ".fits")
        vmap_fitsname = os.path.join(workdir, path_outputs, "vmap_cl" + str(i) + ".fits")
        zslice = {
            "zsl": zcl[i],
            "zsl_min": zcl[i] - 2.0 * fsig(zcl[i]),
            "zsl_max": zcl[i] + 2.0 * fsig(zcl[i]),
        }
        ra_map, dec_map, weight_map = select_galaxies_in_slice(
            dat_galcat,
            galcat_keys,
        ...
        xycat = compute_catimage(
            ra_map, dec_map, weight_map, zslice, wazp_specs, tile_specs, cosmo_params
        )
        map2fits(xycat, wazp_specs, tile_specs, zcl[i], cosmo_params, xycat_fitsname)
        run_mr_filter(xycat_fitsname, wmap_fitsname, wazp_specs)
        wmap_list.append(wmap_fitsname)

        vmap = vmap_from_hpx(dat_footprint, hpx_meta, tile_specs, wazp_specs, cosmo_params, zcl[i])
        map2fits(vmap, wazp_specs, tile_specs, zcl[i], cosmo_params, vmap_fitsname)
        vmap_list.append(vmap_fitsname)

    return wmap_list, vmap_list
...
def update_config(param_cfg, param_data):
    ...
    if param_cfg["admin"]["tiling_pmem"]["ntiles"] == "max":
        param_cfg["admin"]["tiling_pmem"]["ntiles"] = param_cfg["admin"]["slurm"]["max_parallel"]

    # update mag with selected filter in config
    param_data["galcat"][survey]["keys"]["key_mag"] = param_data["galcat"][survey]["keys"][
        "key_mag"
    ][ref_filter]
    # update final cluster cat output
    param_cfg["clcat"]["wazp"]["cat"] = os.path.join(workdir, "tmp", "clusters.fits")
    ...
    param_cfg["pmem_cfg"]["mag_bin_specs"]["max"] = np.float64(int(maglim_pmem) + 1)
