...
from .utils import mad, gaussian, dist_ang, _mstar_
...
from .utils import read_mosaicFitsCat_in_disc, add_hpx_to_cat
from .utils import concatenate_clusters
from .utils import read_sources_in_hpixs
from .utils import read_mosaicFootprint_in_disc, filter_hpx_tile
from .utils import area_ann_deg2, make_tile_cat
...
def nfw_2D(R, Rc, Rs, h):
    ...
    x = R / Rs
    if x > 1.0:
        f = 1.0 - (2.0 / (x**2 - 1.0) ** 0.5) * atan(((x - 1.0) / (x + 1.0)) ** 0.5)
    if x < 1.0:
        f = 1.0 - (2.0 / (1.0 - x**2) ** 0.5) * atanh(((1 - x) / (1.0 + x)) ** 0.5)
    if x == 1.0:
        f = 0.0
...
def get_boundaries(pixel, Nside, nest, racl, ramin, ramax):
    ...
    if racl >= 0.0 and ramin < 0.0:
        ra1 = ra - 360.0
        ra[ra > 180.0] = ra1[ra > 180.0]
    if racl <= 360.0 and ramax > 360.0:
        ra1 = ra + 360.0
        ra[ra <= 180.0] = ra1[ra <= 180.0]
    return ra, dec
...
def hpx_in_radec_window(hpx_meta, ramin0, ramax0, decmin, decmax):
    ...
    """
    Nside, nest = hpx_meta["Nside"], hpx_meta["nest"]
    racen, deccen = (ramin0 + ramax0) / 2.0, (decmin + decmax) / 2.0
    ...
    if ramin0 < 0.0:
        ...
        ra1[idx] = ra0[idx] - 360.0
        ramin = ramin0
        ramax = ramax0

    hpx = pixels_in_disc[(ra1 <= ramax) & (ra1 >= ramin) & (dec0 <= decmax) & (dec0 >= decmin)]
    return hpx
...
def local_footprint(my_cluster, data_fp, hpx_meta, bkg_specs):
    ...
    hpix_local, frac_local, area_deg2, coverfac = hpx_in_annulus(
        racl, deccl, 0.0, radius_deg, data_fp, hpx_meta, inclusive=True
    )
    data_lfp = np.zeros(
        (len(hpix_local)),
        dtype={"names": (hpx_meta["key_pixel"], hpx_meta["key_frac"]), "formats": ("i8", "f8")},
    )
...
def weighted_coverfrac_in_disk(
    ...
    racl, deccl = my_cluster["racl"], my_cluster["deccl"]
    conv_factor = my_cluster["conv_factor"]
    radius_test_deg = np.degrees(weighted_coverfrac_specs["radius_mpc"] / conv_factor)
    hpx_in_disc, frac_in_disc, area_deg2, coverfrac = hpx_in_annulus(
        racl, deccl, 0.0, radius_test_deg, data_fp, hpx_meta, inclusive=False
    )
    pixels_in_fdisc = all_hpx_in_annulus(
        racl, deccl, 0.0, radius_test_deg, hpx_meta, inclusive=False
    )

    # full disc
    ra_in_fdisc, dec_in_fdisc = hp.pix2ang(Nside, pixels_in_fdisc, nest, lonlat=True)
    dist_fdisc_mpc = conv_factor * dist_ang(ra_in_fdisc, dec_in_fdisc, racl, deccl)

    # hpx disc
    ra_in_disc, dec_in_disc = hp.pix2ang(Nside, hpx_in_disc, nest, lonlat=True)
    dist_mpc = conv_factor * dist_ang(ra_in_disc, dec_in_disc, racl, deccl)

    Rc = weighted_coverfrac_specs["radius_mpc"]
    h = cosmo_params["H"] / 100.0
    ...
    weighted_coverfrac = np.sum(np.multiply(frac_in_disc, weight)) / np.sum(fweight)

    return weighted_coverfrac


def compute_cl_coverfracs(my_cluster, weighted_coverfrac_specs, data_fp, hpx_meta, cosmo_params):
    """
    ...
    racl, deccl = my_cluster["racl"], my_cluster["deccl"]
    conv_factor = my_cluster["conv_factor"]
    radius_testcl_deg = np.degrees(weighted_coverfrac_specs["radius_mpc"] / conv_factor)
    hpix, frac, testcl_area_deg2, testcl_coverfrac = hpx_in_annulus(
        racl, deccl, 0.0, radius_testcl_deg, data_fp, hpx_meta, inclusive=False
    )
...
def compute_bkg_coverfracs(bkg_specs, my_cluster, data_fp, data_fp_mask, hpx_meta):
    """
    ...
    hpix, frac, bkg_area_deg2, bkg_coverfrac = hpx_in_annulus(
        racl, deccl, radius_bkg_min_deg, radius_bkg_max_deg, data_fp, hpx_meta, inclusive=False
    )
    hpix, frac, bkg_wmask_area_deg2, bkg_wmask_coverfrac = hpx_in_annulus(
        racl, deccl, radius_bkg_min_deg, radius_bkg_max_deg, data_fp_mask, hpx_meta, inclusive=False
    )
...
def plot_footprint(
    ...
    hpix, frac = data_fp_plot[hpx_meta["key_pixel"]], data_fp_plot[hpx_meta["key_frac"]]
    ...
    dra10, ddec10 = (
        np.degrees(dist_ang(ra10, dec10, racl, dec10)),
        np.degrees(dist_ang(racl, dec10, racl, deccl)),
    )
    dra09, ddec09 = (
        np.degrees(dist_ang(ra09, dec09, racl, dec09)),
        np.degrees(dist_ang(racl, dec09, racl, deccl)),
    )
    dra08, ddec08 = (
        np.degrees(dist_ang(ra08, dec08, racl, dec08)),
        np.degrees(dist_ang(racl, dec08, racl, deccl)),
    )

    circle_bkg_max = get_circ(racl, deccl, 60.0 * radius_bkgmax_deg, 50)
    circle_bkg_min = get_circ(racl, deccl, 60.0 * radius_bkgmin_deg, 50)
    circle = get_circ(racl, deccl, 60.0 * radius_testcl_deg, 50)
    ...
    fig, ax = plt.subplots()
    ax.set_aspect(1.0 / np.cos(np.radians(deccl)))

    for p in pix10[:]:
        plt.fill(
            *get_boundaries(p, Nside_plot, nest, racl, ramin, ramax), color="r", alpha=0.6, zorder=1
        )
    for p in pix09[:]:
        plt.fill(
            *get_boundaries(p, Nside_plot, nest, racl, ramin, ramax), color="r", alpha=0.4, zorder=1
        )
    for p in pix08[:]:
        plt.fill(
            *get_boundaries(p, Nside_plot, nest, racl, ramin, ramax), color="r", alpha=0.2, zorder=1
        )

    plt.plot(*circle_bkg_max, color="b", zorder=8, label="bkg region")
    plt.plot(*circle_bkg_min, color="b", zorder=8)
    plt.plot(*circle, color="g", zorder=8, label=str(round(radius_testcl_mpc, 1)) + " Mpc")
    plt.axis([ramax, ramin, decmin, decmax])
    plt.title(
        "cl. id = "
        + str(idcl).zfill(6)
        + "    coverfrac ( bkg / cl / cl+w ) = "
        + str(round(bkg_cfc, 2))
        + " / "
        + str(round(cl_cfc, 2))
        + " / "
...
def plot_richness_vec(
    radius_vec_mpc, richness_vec, richness_err_vec, rfit, r200, idcl, workdir, out_paths
):
    """
    ...
    plt.plot(
        radius_vec_mpc,
        10 ** (rfit(np.log10(radius_vec_mpc))),
        color="red",
        label="linear fit / slope = " + str(round(slope, 3)),
    )
    plt.axvline(
        x=r200, color="red", linestyle="--", label="$R_{200} =$" + str(round(r200, 2)) + " Mpc"
    )
    plt.xscale("log")
    plt.yscale("log")
    plt.xlabel("R (Mpc)")
    ...
    plt.legend(loc=2)
    plt.title(
        "cluster id = " + str(idcl).zfill(6) + "    N$_{200}=$" + str(round(n200_pmem_cor, 2)),
        fontsize=10,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(workdir, out_paths["pmem"]["plots"], "richness_vec_cl" + str(idcl) + ".png"),
        dpi=300,
    )
    return
...
def plot_pmems(my_cluster, data_gal, galcat_keys, pmem, r200, n200, out_paths):
    ...
    circle1 = get_circ(racl, deccl, 60.0 * r1_deg, 100)
    circle2 = get_circ(racl, deccl, 60.0 * r2_deg, 100)
    circle3 = get_circ(racl, deccl, 60.0 * r3_deg, 100)
    circle4 = get_circ(racl, deccl, 60.0 * r4_deg, 100)
    circle_r200 = get_circ(racl, deccl, 60.0 * r200_deg, 100)

    data_gal[galcat_keys["key_mag"]]
    ...
    plt.plot(*circle1, color="black", linewidth=0.5, zorder=1, label="0.5 Mpc separation")
    ...
    plt.ylabel("Dec. (deg)")
    plt.title(
        "cluster id = " + str(idcl).zfill(6) + "    N$_{200}=$" + str(round(n200, 2)),
        fontsize=10,
        color="b",
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(workdir, out_paths["pmem"]["plots"], "pmems_cl" + str(idcl) + ".png"), dpi=300
    )
    return
...
def sigma_dz0(zp_metrics, z):
    ...
    sig0 = sig_dz0[0] + sig_dz0[1] * z + sig_dz0[2] * z**2 + sig_dz0[3] * z**3
    if np.ndim(sig0) == 0:
        if z > zpsat:
            sig0 = sig_dz0[0] + sig_dz0[1] * zpsat + sig_dz0[2] * zpsat**2 + sig_dz0[3] * zpsat**3
    else:
        sig0[z > zpsat] = (
            sig_dz0[0] + sig_dz0[1] * zpsat + sig_dz0[2] * zpsat**2 + sig_dz0[3] * zpsat**3
        )
    return sig0
...
def filter_galcat(
    ...
    sigma_dz0(zp_metrics, zclmax)
    zclmax + photoz_support["nsig"] * sigma_dz0(zp_metrics, zclmax) * (1.0 + zclmax)
    mag = data_gal[galcat_keys["key_mag"]]
    zp = data_gal[galcat_keys["key_zp"]]
    zpm = (zp + photoz_support["nsig"] * sigma_dz0(zp_metrics, zclmax)) / (
        1.0 - photoz_support["nsig"] * sigma_dz0(zp_metrics, zclmax)
    )
...
def init_richness_output(data_cls_analysis, richness_specs, clcat_analysis_keys):
    """
    ...
    data_richness = np.zeros(
        (len(data_cls_analysis)),
        dtype={
            "names": (
...
def Gaussian_range_pos(x, eux, edx, dx, allx, sigma_unit=3.0):
    """
    ...
    deltad = sigma_unit * np.sqrt(dx**2.0 + edx**2.0)
    deltau = sigma_unit * np.sqrt(dx**2.0 + eux**2.0)
    ...
    result = np.core.records.fromarrays(list(result.values()), names=list(result.keys()))
...
def Gaussian_pdf(x, eux, edx, allx, dx, startpos, endpos):
    ...
    pdf = np.array([0.0] * len(_x))  # aux pdf for x-value of AUXx
    wd = _x < x
    pdf[wd] = np.exp(
        -((_x[wd] - x) ** 2.0) / (2.0 * (edx**2.0 + dx**2.0))
    )  # difference between bins above and below the obs value x
    pdf[~wd] = np.exp(-((_x[~wd] - x) ** 2.0) / (2.0 * (eux**2.0 + dx**2.0)))
    pdf = pdf * dx  # Integration over the bin
    if sum(pdf) != 0.0:
        pdf = pdf / sum(pdf)  # normalization equal to 1. Integration is truncated at 3\sigma
...
def Kernel_pdf(errx, dx, pos, vecmin, vecmax):
    ...
    result = {
        "pos": xpos[(xpos >= 0) & (xpos <= nbin - 1)],
        "pdf": gkernel[(xpos >= 0) & (xpos <= nbin - 1)],
    }
    result = np.core.records.fromarrays(list(result.values()), names=list(result.keys()))
    return result
...
def vec_bin_pos(vec, vecbin_specs):
    ...
    """
    vecmin, _vecmax = vecbin_specs["min"], vecbin_specs["max"]
    step = vecbin_specs["step"]
...
def rad_bin_pos(dist2cl, radial_bin_specs):
    ...
    indf = (np.log10(dist2cl) - lradb[0] - lrad_step / 2.0) / lrad_step
    index = indf.astype(int) + 1
...
def bkg_mag_zp_counts(
    ...
    counts_bkg_arcmin2 = counts_bkg / (3600.0 * area_bkg_deg2)
...
def density_fct(data, datamin, datamax, npts):
    histo, bins = np.histogram(
        data, bins=npts, range=(datamin, datamax), weights=None, density=True
    )
    xx = (bins[0 : len(bins) - 1] + bins[1 : len(bins)]) / 2.0
    gauss_kernel = Gaussian1DKernel(2)
    density = convolve(histo, gauss_kernel, boundary="extend")
    return xx, density


def mean_bkg(my_cluster, counts_bkg_mpc2, counts_bkg_arcmin2, mag_bin_specs, richness_specs):
    """
    ...
    md_bkg_mpc2 = np.sum(
        counts_bkg_mpc2[vec_bin_pos(magmin, mag_bin_specs) : vec_bin_pos(magmax, mag_bin_specs)]
    )
    md_bkg_arcmin2 = np.sum(
        counts_bkg_arcmin2[vec_bin_pos(magmin, mag_bin_specs) : vec_bin_pos(magmax, mag_bin_specs)]
    )
    return md_bkg_mpc2, md_bkg_arcmin2


def build_my_cluster(
    index_cl,
    data_cluster,
    clcat_analysis_keys,
    photoz_support,
    ...
    racl, deccl = (
        data_cluster[clcat_analysis_keys["key_ra"]],
    ...
    zpmin = zcl - photoz_support["nsig"] * sigma_dz0(zp_metrics, zcl) * (1.0 + zcl)
    zpmax = zcl + photoz_support["nsig"] * sigma_dz0(zp_metrics, zcl) * (1.0 + zcl)
    sig_dz = sigma_dz0(zp_metrics, zcl) * (1.0 + zcl)
    ...
    slope_dprofile = (
        pmem_specs["slope_dprofile_poly"][0] + pmem_specs["slope_dprofile_poly"][1] * zcl
    )

    my_cluster = {
        "idcl": str(idcl),
        "index_cl": index_cl,
        "racl": np.float64(racl),
        "deccl": np.float64(deccl),
        "zcl": zcl,
...
def profile_mpc(
    ...
    for i in range(len(radius)):
        ...
        hpix, frac, area, annulus_area_fraction = hpx_in_annulus(
            racl, deccl, 0.0, radius_cl_max_deg, data_fp, hpx_meta, inclusive=False
        )
        area_cl_mpc2 = area_ann_deg2(0.0, radius_cl_max_mpc)
        ind_cl = (dist2cl_mpc < radius_cl_max_mpc) & (dist2cl_mpc >= radius_previous)
        magb, cld_ = binned_counts(mag[ind_cl], mag_bin_specs, pmem_specs)
        if annulus_area_fraction > 0.0 and pmem_specs["area_correction"]:
        ...
        cl_dens = np.sum(
            cl_mpc2[vec_bin_pos(magmin, mag_bin_specs) : vec_bin_pos(magmax, mag_bin_specs)]
        )
        md_cl_2d[i] = cl_dens - md_bkg_2d
        md_cl_3d[i] = md_cl_2d[i] / (2.0 * radius_cl_max_mpc)

    return md_cl_2d, md_cl_3d  # densities in /mpc2 and /mpc3


def clusters_periphery(my_cluster, data_cls, clcat_keys, periphery_specs):
    racl, deccl, zcl = my_cluster["racl"], my_cluster["deccl"], my_cluster["zcl"]
    conv_factor = my_cluster["conv_factor"]
    sig_dz = my_cluster["sig_dz"]

    nsig_msk, nsig_los = periphery_specs["nsig_msk"], periphery_specs["nsig_los"]
    radius_msk_mpc, rad_los_mpc = periphery_specs["radius_msk_mpc"], periphery_specs["rad_los_mpc"]
    radmin_msk_mpc, radmax_msk_mpc = (
        periphery_specs["radmin_msk_mpc"],
        periphery_specs["radmax_msk_mpc"],
    )

    ra, dec, z = (
        data_cls[clcat_keys["key_ra"]],
...
def footprint_with_cl_masks(my_cluster, data_cls, clcat_keys, periphery_specs, data_fp, hpx_meta):
    """
    ...
    if len(data_cls[cond_snr]) > 0:
        ...
        racl_out, deccl_out = (
            data_cls_msk[clcat_keys["key_ra"]],
            data_cls_msk[clcat_keys["key_dec"]],
        )
        ncl_masked = len(racl_out)
        if (len(racl_out)) > 0:
            radius_clmask_deg = np.full(len(racl_out), np.degrees(cl_mask_radius_mpc / conv_factor))
            hpix_clmask = hpix_in_discs(racl_out, deccl_out, radius_clmask_deg, hpx_meta)
            ind_out = np.isin(
    ...
    data_lfp_mask = np.zeros(
        (len(hpix_map_with_clmask)),
        dtype={"names": (hpx_meta["key_pixel"], hpx_meta["key_frac"]), "formats": ("i8", "f8")},
    )
...
def compute_r200_n200(
    ...
    idcl = my_cluster["idcl"]
    _racl, _deccl = my_cluster["racl"], my_cluster["deccl"]
    zcl = my_cluster["zcl"]
    conv_factor = my_cluster["conv_factor"]
    mstar = my_cluster["mstar"]
    zpmin, zpmax = my_cluster["zpmin"], my_cluster["zpmax"]
    ...
    om = cosmo.Om(zcl)
    Delta = (18.0 * np.pi**2 + 82.0 * (om - 1.0) - 39.0 * (om - 1.0) ** 2) / om
    # see Eke et al. 1996   + Bryan & Norman 1998 / Mo et al CUP

    magmin, magmax = mstar - richness_specs["dmag_bright"], mstar + richness_specs["dmag_faint"]
    if magmin < mag_bin_specs["min"]:
        magmin = mag_bin_specs["min"]
    if magmax > mag_bin_specs["max"]:
        magmax = mag_bin_specs["max"]
    ...
    md_bkg_2d = np.sum(
        bkg_mpc2[vec_bin_pos(magmin, mag_bin_specs) : vec_bin_pos(magmax, mag_bin_specs)]
    )
    md_bkg_3d = md_bkg_2d / cosmo.angular_diameter_distance_z1z2(zpmin, zpmax).value
    r200, n200, flag_r200 = 0.0, 0.0, 0
    slope_3d_mean = my_cluster["slope_dprofile"] - 1.0
    no_failure = True
    ...
    if md_bkg_2d > 0.0:
        ...
        if len(ratio[cond_init]) > 2:
            xtrain, ytrain = np.log10(radius_test[cond_init]), np.log10(ratio[cond_init])
            smoo = convolve(ytrain, gauss_kernel, boundary="extend")
            fradius = interpolate.interp1d(
                smoo, xtrain, kind="linear", bounds_error=False, fill_value="extrapolate"
            )
            r200_init = 10 ** (fradius(np.log10(Delta)))
            cond_fit = (ratio > 0.0) & (radius_test <= r200_init)

            if len(radius_test[cond_fit]) > 1:
                xtrain, ytrain = np.log10(radius_test[cond_fit]), np.log10(ratio[cond_fit])
                ...
                if pmem_specs["dprofile_forced"]:
                    ...
                    if (slope_3d < pmem_specs["slope_free_min"] - 1.0) or (
                        slope_3d > pmem_specs["slope_free_max"] - 1.0
                    ):
                        slope_3d = slope_3d_mean
                        y0 = np.mean(ytrain - slope_3d * xtrain)
                        rfit[0] = y0
        ...
        np.degrees(r200 / conv_factor)
        n200 = cl_dens_r200 * np.pi * r200**2
        my_cluster["slope_dprofile"] = slope_3d + 1.0
...
def plot_mag_counts(
    ...
    my_cluster["imag_star"]

    rad1, rad2 = 0.0, pmem_specs["radius_densnorm"]
    np.degrees(rad1 / conv_factor)
    np.degrees(rad2 / conv_factor)
    cond1 = (magb > magmin) & (magb < magmax)

    plt.clf()
    plt.plot(magb[cond1], bkg_arcmin2[cond1], linestyle="-", color="blue", label="background")
    plt.plot(
        magb[cond1], cl_local_cor[cond1], linestyle="--", color="green", label="cluster - effective"
    )
    plt.scatter(
        magb[cond1],
        cl_local[cond1],
    ...
    plt.tight_layout()
    plt.yscale("log")
    plt.axis([magmin, magmax, 10 ** (-6.0), max(cl_local_cor) * 5])
...
def plot_raw_mag_counts(magb, bkg_arcmin2, cl_arcmin2, cl_cor, mstar, magmin, magmax, output):
    """
    ...
    ymin, ymax = 0.90 * min(bkg_arcmin2[cond1]), 10.0 * max(bkg_arcmin2[cond1])

    plt.clf()
    plt.plot(magb[cond1], bkg_arcmin2[cond1], linestyle="-", color="blue", label="backgrountled")
    plt.plot(magb[cond1], cl_arcmin2[cond1], linestyle="--", color="red", label="cluster")
    plt.plot(magb[cond1], cl_cor[cond1], linestyle="-", color="red", label="cluster cor")
    plt.axvline(x=mstar, color="red", linestyle="--", label="$m^*$")
    plt.xlabel("$mag$")
    plt.legend(loc=2)
    plt.tight_layout()
...
def prepare_data_calib_dz(my_cluster, pmem_cfg, hpx_meta, data_gal, galcat_keys):
    ...
    if ng > 0:
        ...
        ra, dec = data_galc[galcat_keys["key_ra"]], data_galc[galcat_keys["key_dec"]]
        zp, mag = data_galc[galcat_keys["key_zp"]], data_galc[galcat_keys["key_mag"]]
        racl, deccl, zcl = my_cluster["racl"], my_cluster["deccl"], my_cluster["zcl"]
        conv_factor = my_cluster["conv_factor"]
        dcen = conv_factor * dist_ang(ra, dec, racl, deccl)
        mag - my_cluster["mstar"]
        data_for_calib = np.zeros(
            (ng),
            dtype=[
...
def local_zp_stats(pmem_cfg, data_gal_tile, galcat_keys, my_cluster):
    ...
    data_gal_zpstats = general_local_galcat(
        pmem_cfg, data_gal_tile, galcat_keys, my_cluster, 0.5, nsig, 2.0, 2.0
    )
    zp = data_gal_zpstats[galcat_keys["key_zp"]]

    zcl, sig_dz = my_cluster["zcl"], my_cluster["sig_dz"]
    _zpmin, _zpmax = zcl - nsig * sig_dz / (1.0 + zcl), zcl + nsig * sig_dz / (1.0 + zcl)
    zpmin2, zpmax2 = zcl - 2.0 * sig_dz, zcl + 2.0 * sig_dz

    zpmed = np.median(zp[(zp < zpmax2) & (zp > zpmin2)])
    x = (zp - zpmed) / (1.0 + zpmed)
    std0 = mad(x)
...
def plot_pdz(my_cluster, zb, pdz, zpmed, output):
    ...
    zpmin5, zpmax5 = zcl - 5.0 * sig_dz, zcl + 5.0 * sig_dz
    zpmin, zpmax = zcl - 2.0 * sig_dz, zcl + 2.0 * sig_dz
    ...
    plt.axvline(x=zpmin, color="green", linestyle="--")
    plt.axvline(x=zpmax, color="green", linestyle="--", label=" +/- 2$\sigma$")
    plt.title("cl. id = " + str(idcl).zfill(6) + "    zcl = " + str(round(zcl, 2)), fontsize=8)
    plt.legend()
    plt.savefig(output, dpi=300)
    return


def cl_dens_ann_arcmin(
    my_cluster,
    data_gal,
    galcat_keys,
    radius_cl_min_deg,
    ...
    in_ann = (dist2cl_deg <= radius_cl_max_deg) & (dist2cl_deg >= radius_cl_min_deg)
    hpix, frac, area, annulus_area_fraction = hpx_in_annulus(
        racl, deccl, radius_cl_min_deg, radius_cl_max_deg, data_fp_mask, hpx_meta, inclusive=False
    )
    if len(mag[in_ann]) > 0:
        ...
        cl_arcmin2 = (
            _cl_ / (3600.0 * area_ann_deg2(radius_cl_min_deg, radius_cl_max_deg)) - bkg_arcmin2
        )
        cond1 = (
            (cl_arcmin2 >= bkg_arcmin2)
            & (bkg_arcmin2 > 0.0)
        ...
        cl_cor = np.maximum(cl_arcmin2, mean_cl)
        cl_cor[cl_cor < 0.0] = 0.0
        cl_arcmin2[cl_arcmin2 < 0.0] = 0.0
    else:
    ...
    return cl_cor, cl_arcmin2, 3600.0 * area_ann_deg2(radius_cl_min_deg, radius_cl_max_deg)
...
def varGaussian_pdf(x, sig0, allx, dx, startpos, endpos):
    ...
    ex = np.sqrt(
        (sig0 * (1.0 + _x)) ** 2.0 + dx**2.0
    )  # 0th order approximation (in zp-zs, i.e. allx-x)  for the \sigma, where convolution with Gaussian with sigma = dx is applied
    pdf = np.exp(-((_x - x) ** 2.0) / (2.0 * (ex**2.0))) / ex  # var gaussian pdf
    if sum(pdf) != 0.0:
        pdf = pdf / sum(pdf)  # normalization equal to the unit. Integration truncated at 3\sigma
    result = {"pos": xpos, "pdf": pdf}
    result = np.core.records.fromarrays(
        list(result.values()), names=list(result.keys())
    )  # dictionary => structured array
...
def general_local_galcat(
    ...
    _ra, _dec = data_gal_tile[galcat_keys["key_ra"]], data_gal_tile[galcat_keys["key_dec"]]
    hpix_gal = data_gal_tile["hpix_tmp"]
    zp, mag = data_gal_tile[galcat_keys["key_zp"]], data_gal_tile[galcat_keys["key_mag"]]
    # extract cluster data
    racl, deccl, zcl = my_cluster["racl"], my_cluster["deccl"], my_cluster["zcl"]
    conv_factor = my_cluster["conv_factor"]
    radius_bkg_max_deg = np.degrees(radius_max_mpc / conv_factor)
    mstar = my_cluster["mstar"]
    zpmin, zpmax = zcl - nsig * my_cluster["sig_dz"], zcl + nsig * my_cluster["sig_dz"]
    ...
    in_cone = cond_in_hpx_disc(
        hpix_gal[cond], pmem_cfg["Nside_tmp"], pmem_cfg["nest_tmp"], racl, deccl, radius_bkg_max_deg
    )
...
def local_galcat(
    ...
    local_cat = general_local_galcat(
        pmem_cfg,
        data_gal_tile,
        galcat_keys,
        hpx_meta,
    ...
    racl, deccl, _zcl = my_cluster["racl"], my_cluster["deccl"], my_cluster["zcl"]
    conv_factor = my_cluster["conv_factor"]
    radius_bkg_min_deg = np.degrees(radius_min_mpc / conv_factor)
    np.degrees(radius_max_mpc / conv_factor)
    ...
    dist2cl_mpc[in_cone] = conv_factor * dist_ang(ra[in_cone], dec[in_cone], racl, deccl)
...
def local_galcat_zp_test(data_gal_tile, galcat_keys, my_cluster, hpx_meta):
    ...
    ra0, dec0 = data_gal_tile[galcat_keys["key_ra"]], data_gal_tile[galcat_keys["key_dec"]]
    ...
    hpix_gal0 = hp.ang2pix(hpx_meta["Nside"], ra0, dec0, hpx_meta["nest"], lonlat=True)
    racl, deccl = my_cluster["racl"], my_cluster["deccl"]
    zcl = my_cluster["zcl"]
    mstar = my_cluster["mstar"]
    conv_factor = my_cluster["conv_factor"]
    ...
    zpmin, zpmax = zcl - 3.0 * sig_dz, zcl + 3.0 * sig_dz
    magmin, magmax = mstar - 2.0, mstar + 2.0

    # select galaxies only in the cluster cone and with a mag_cut
    hpx_cone = all_hpx_in_annulus(racl, deccl, 0.0, radius_bkg_max_deg, hpx_meta, inclusive=True)
...
def normalized_central_density(my_cluster, pmem_specs, mdens):
    ...
    if math.isclose(slope, -2.0, rel_tol=1e-5):
        d0 = mdens / (1.0 + 2.0 * np.log(R0 / Rc))
    else:
        d0 = mdens * (slope / 2.0 + 1.0) / (1.0 + 0.5 * slope * (Rc / R0) ** (slope + 2.0))
...
def ntot_nbkg(
    ...
    for im in range(0, len(magb)):
        magmin, magmax = (
            max(mag_bin_specs["min"], magb[im] - 0.5),
            min(mag_bin_specs["max"], magb[im] + 0.5),
        )
        cl_dens = np.sum(
            cl_arcmin2_cor[vec_bin_pos(magmin, mag_bin_specs) : vec_bin_pos(magmax, mag_bin_specs)]
        )
        bkg_dens[im] = np.sum(
            bkg_arcmin2[vec_bin_pos(magmin, mag_bin_specs) : vec_bin_pos(magmax, mag_bin_specs)]
        )
    ...
    area_arcmin2 = area_ref * 3600.0 / np.radians(conv_factor) ** 2
    ncl = area_arcmin2 * (d0[imag] * (dist2cl_mpc / rrr0) ** (slope_dprofile))
    nbkg_cl = area_arcmin2 * bkg_dens[imag]
    ntot = np.maximum(ncl + nbkg_cl, 1.0)
    nbkg = np.maximum(bkg_area_arcmin2 * bkg_dens[imag], 1.0)

    return ntot, nbkg


def cl_density_ratio_drad_dmag(
    my_cluster,
    data_gal,
    galcat_keys,
    radial_bin_specs,
    ...
    d0 = np.zeros(len(magb))
    for im in range(0, len(magb)):
        d0[im] = normalized_central_density(my_cluster, pmem_specs, cl_arcmin2_cor[im])
    imag = vec_bin_pos(mag, mag_bin_specs)

    bkg_arcmin2[bkg_arcmin2 <= 0.0] = (
        0.00001  # patch to avoid pmem = nan when BCGs are not represented in bkg
    )
    dist2cl_mpc[dist2cl_mpc < radmin_cc] = radmin_cc
    cl_local_overdensity = (d0[imag] * (dist2cl_mpc / rrr0) ** (slope_dprofile)) / bkg_arcmin2[imag]
...
def compute_pmem(
    ...
    bkg_density_zp = interpolate.interp1d(
        zpb, bkg_zdensity, kind="linear", bounds_error=False, fill_value="extrapolate"
    )
    cl_local_overdensity_zp = cl_local_overdensity * gaussian(zp, zpcl, sig_dz) / bkg_density_zp(zp)

    sigcl_dz = photoz_support["sigcl_over_sigg"] * (zpmax - zpmin) / (2.0 * photoz_support["nsig"])

    PgPc = np.exp(
        -((zp - zpcl) ** 2) / (2.0 * (sig_dz**2 + sigcl_dz**2))
    )  # normalized convolution in Castignani & Benoist 2016

    pmem = (cl_local_overdensity_zp / (1.0 + cl_local_overdensity_zp)) * PgPc
    pmem[(cl_local_overdensity_zp <= 0.0)] = 0.0
    ...
    pmem_err = (1.0 - pmem) * (1.0 / np.sqrt(nbkg) + 1.0 / np.sqrt(ntot))
...
def generate_output_radii(my_cluster, radial_bin_specs, richness_specs):
    ...
    radb = 10**lradb
    radb_arcmin = 60.0 * np.degrees(radb / conv_factor)
    return np.around(radb, 3), np.around(radb_arcmin, 3)


def compute_richness_vec(
    pmem_cfg, pmem0, pmem_err0, data_gal0, galcat_keys, r200, my_cluster, data_fp, hpx_meta
):
    """
    ...
    radb, radius_vec_arcmin = generate_output_radii(my_cluster, radial_bin_specs, richness_specs)

    cond_richness = (
        (data_gal0["dist2cl_mpc"] / r200 < pmem_specs["dmax"])
        & (data_gal0[galcat_keys["key_mag"]] < my_cluster["mstar"] + richness_specs["dmag_faint"])
        & (data_gal0[galcat_keys["key_mag"]] > my_cluster["mstar"] - richness_specs["dmag_bright"])
    )
    ...
    for i in range(0, len(radb)):
        if i == 0:  # no area correction
            ...
            drich = np.sum(pmem[(dist2cl_mpc <= radb[i]) & (dist2cl_mpc > radb[i - 1])])
            drich_err2 = np.sum(
                pmem_err[(dist2cl_mpc <= radb[i]) & (dist2cl_mpc > radb[i - 1])] ** 2
            )

            hpix, frac, area, annulus_area_fraction = hpx_in_annulus(
                racl, deccl, radb_deg[i - 1], radb_deg[i], data_fp, hpx_meta, inclusive=False
            )

            if annulus_area_fraction > 0.0 and pmem_specs["area_correction"]:
                rich[i] = rich[i - 1] + drich / annulus_area_fraction
                rich_err2[i] = rich_err2[i - 1] + drich_err2 / annulus_area_fraction
            else:
                rich[i] = rich[i - 1] + drich
                rich_err2[i] = rich_err2[i - 1] + drich_err2

    rich_err = np.sqrt(rich_err2)
    xr = radb[(radb <= r200 * 1.2) & (rich > 0.0)]
    yr = rich[(radb <= r200 * 1.2) & (rich > 0.0)]

    if len(xr) > 2:
        rfit = np.poly1d(np.polyfit(np.log10(xr[yr > 0]), np.log10(yr[yr > 0]), 1))
    else:
        rfit = [0.0, 1.0]
    return rich, rich_err, radb, radius_vec_arcmin, rfit


def ngals_cor_from_pmems(
    pmem_cfg, pmem0, pmem_err0, data_gal0, galcat_keys, r200, my_cluster, data_fp, hpx_meta
):
    """
    Computes the richness (and errors) as a sum of Pmems in a set of radii
    ...
    radb, radius_vec_arcmin = generate_output_radii(my_cluster, radial_bin_specs, richness_specs)
    cond_richness = (
        (data_gal0["dist2cl_mpc"] / r200 <= 1.0)
        & (data_gal0[galcat_keys["key_mag"]] < my_cluster["mstar"] + richness_specs["dmag_faint"])
        & (data_gal0[galcat_keys["key_mag"]] > my_cluster["mstar"] - richness_specs["dmag_bright"])
    )
        ...
        if i == 0:
            ...
            if r200 > radb_deg[i - 1]:
                ...
                if annulus_area_fraction > 0.0 and pmem_specs["area_correction"]:
                    rich[i] = (
                        rich[i - 1]
                        + np.sum(pmem[(dist2cl_mpc <= radb[i]) & (dist2cl_mpc > radb[i - 1])])
                    ...
                    rich[i] = rich[i - 1] + np.sum(
                        pmem[(dist2cl_mpc <= radb[i]) & (dist2cl_mpc > radb[i - 1])]
                    )
                    rich_err2[i] = rich_err2[i - 1] + np.sum(
                        pmem_err[(dist2cl_mpc <= radb[i]) & (dist2cl_mpc > radb[i - 1])] ** 2
                    )
...
def n200_raw_from_pmems(pmem, data_gal, galcat_keys, my_cluster, richness_specs, r200):
    """
    n200 from Pmems without area correction
    """
    cond_richness = (
        (data_gal["dist2cl_mpc"] / r200 <= 1.0)
        & (data_gal[galcat_keys["key_mag"]] < my_cluster["mstar"] + richness_specs["dmag_faint"])
        & (data_gal[galcat_keys["key_mag"]] > my_cluster["mstar"] - richness_specs["dmag_bright"])
    )
...
def degraded_footprint(data_fp, hpx_meta, Nside_plot):
    ...
    hpix_map, _frac_map = data_fp[hpx_meta["key_pixel"]], data_fp[hpx_meta["key_frac"]]
    ra, dec = hp.pix2ang(hpx_meta["Nside"], hpix_map, hpx_meta["nest"], lonlat=True)
...
def compute_Nside_plot(hpx_meta, radius_deg, nstep):
    ...
    size_pix_deg = hp.nside2resol(Nside_arr.astype(int), arcmin=True) / 60.0
    Nside_plot = int(Nside_arr[np.abs(radius_deg / float(nstep) - size_pix_deg).argmin()])
    return Nside_plot


def data_gal_tile_from_mosaic(galcat_mosaic_specs, galcat_keys, my_cluster, radius_max_mpc):
    """
    ...
    return read_mosaicFitsCat_in_disc(galcat_mosaic_specs, galcat_keys, racl, deccl, radius_max_deg)
...
def pmem_tile(
    ...
    if admin["target_mode"]:
        ...
        tile_dir = os.path.join(workdir, "tiles", "tile_" + str(int(tile_specs["id"])).zfill(5))
        print("..... Tile ", int(tile_specs["id"]))

    if os.path.isfile(os.path.join(tile_dir, out_paths["pmem"]["results"], "clusters.fits")):
        return
    ...
    if dat_galcat is None:
        data_gal_tile, data_fp_tile = make_tile_cat(tile_specs, galcat, footprint, maglim, pmem_cfg)
        if verbose >= 2:
    ...
    if len(data_cls_tile) > 0:
        if os.path.isfile(os.path.join(tile_dir, out_paths["pmem"]["results"], "richness.fits")):
            return
    ...
    for i in range(0, len(data_cls_analysis_tile)):
        ...
        if verbose >= 1:
            ...
            print(
                ...
                + str(i + 1)
                + "/"
                + str(len(data_cls_analysis_tile))
                + " )   Cluster ID = "
                + str(idcl)
                + "      ra = "
                + str(round(racl, 3))
        ...
        if (
            zcl < pmem_cfg["global_conditions"]["zcl_min"]
            or zcl > pmem_cfg["global_conditions"]["zcl_max"]
        ):
        ...
        my_cluster = build_my_cluster(
            ...
            data_cluster,
            clcat["keys"],
            pmem_cfg["photoz_support"],
            zp_metrics,
            pmem_cfg["mag_bin_specs"],
            pmem_cfg["pmem_specs"],
            pmem_cfg["richness_specs"],
            mstar_filename,
            cosmo_params,
        )
        ...
        if len(data_lgal) == 0:
            ...
            continue
        data_lfp = local_footprint(my_cluster, data_fp_tile, footprint, pmem_cfg["bkg_specs"])
        data_lfp_mask, ncl_masked = footprint_with_cl_masks(
            my_cluster,
            data_cls_tile,
            clcat["keys"],
            pmem_cfg["periphery_specs"],
            data_lfp,
            footprint,
        )
        if verbose >= 1:
        ...
        cl_cfc, cl_wcfc = compute_cl_coverfracs(
            my_cluster, pmem_cfg["weighted_coverfrac_specs"], data_lfp, footprint, cosmo_params
        )

        bkg_cfc, bkg_wmask_cfc, bkg_area_deg2 = compute_bkg_coverfracs(
            pmem_cfg["bkg_specs"], my_cluster, data_lfp, data_lfp_mask, footprint
        )

        if verbose >= 1:
            print(
                "    Cluster coverage (%)    raw = "
                + str(round(100.0 * cl_cfc, 1))
                + "     weighted = "
                + str(round(100.0 * cl_wcfc, 1))
            )
            print(
                "    Bkg     coverage (%)    raw = "
                + str(round(100.0 * bkg_cfc, 1))
                + " with cl.masks = "
                + str(round(100.0 * bkg_wmask_cfc, 1))
            )
        ...
        data_richness["raw_coverfrac"][i] = round(100.0 * cl_cfc, 1)
        data_richness["weighted_coverfrac"][i] = round(100.0 * cl_wcfc, 1)
        data_richness["bkg_raw_coverfrac"][i] = round(100.0 * bkg_cfc, 1)
        data_richness["bkg_coverfrac"][i] = round(100.0 * bkg_wmask_cfc, 1)

        if 100.0 * cl_wcfc < pmem_cfg["global_conditions"]["cl_cover_min"]:
            data_richness["flag_pmem"][i] = 3
            continue

        if 100.0 * bkg_wmask_cfc < pmem_cfg["global_conditions"]["bkg_cover_min"]:
        ...
        if data_members is not None:
            if flag_init_members == 0:
                ...
                data_members_tile = np.hstack((data_members_tile, data_members))

        # in calib_dz mode :
        # produce list of galaxies in 1Mpc cylinders
        # around clusters with SNR>SNRlim
        if pmem_cfg["calib_dz"]["mode"] and my_cluster["snr_cl"] > pmem_cfg["calib_dz"]["snr_min"]:
            data_for_calib = prepare_data_calib_dz(
                my_cluster, pmem_cfg, footprint, data_gal_tile, galcat["keys"]
            )
            if data_for_calib is not None:
                if flag_init_calib == 0:
                    ...
                    data_for_calib_tile = np.hstack((data_for_calib_tile, data_for_calib))
            # write calib file
            t = Table(data_for_calib_tile)
            t.write(
                os.path.join(out_paths["workdir_loc"], pmem_cfg["calib_dz"]["filename"]),
                overwrite=True,
            )

    # write outputs to fits
    t = Table(data_richness)
    t.write(os.path.join(tile_dir, out_paths["pmem"]["results"], "richness.fits"), overwrite=True)
    t = Table(data_members_tile)
    t.write(os.path.join(tile_dir, out_paths["pmem"]["results"], "pmem.fits"), overwrite=True)
...
def pmem_list(
    ...
    # Initialize output table "richness"
    data_richness = init_richness_output(data_cls_analysis, pmem_cfg["richness_specs"], clcat_keys)
    ...
    for i in range(0, len(data_cls_analysis)):
        ...
        field = np.core.records.fromarrays(
            list(field.values()), names=list(field.keys())
        )  # dict. in a np struct.array

        data_gal = read_mosaicFitsCat_in_disc(galcat, field, radius_field_deg)
        ...
        data_fp = read_mosaicFootprint_in_disc(hpx_meta, field, radius_field_deg)
        ...
        if len(data_lgal) == 0:
            data_richness["flag_pmem"][i] = 3
            continue
        data_lfp = local_footprint(my_cluster, data_fp, hpx_meta, pmem_cfg["bkg_specs"])
        data_lfp_mask, ncl_masked = footprint_with_cl_masks(
            my_cluster, data_cls_all, clcat_keys, pmem_cfg["periphery_specs"], data_lfp, hpx_meta
        )
        if verbose >= 1:
            print("    Nr of masked clusters in periphery : ", ncl_masked)
        ...
        # test cluster and bkg coverage
        cl_cfc, cl_wcfc = compute_cl_coverfracs(
            my_cluster, pmem_cfg["weighted_coverfrac_specs"], data_lfp, hpx_meta, cosmo_params
        )

        bkg_cfc, bkg_wmask_cfc, bkg_area_deg2 = compute_bkg_coverfracs(
            pmem_cfg["bkg_specs"], my_cluster, data_lfp, data_lfp_mask, hpx_meta
        )
        ...
        if verbose >= 2:
            plot_footprint(
                ...
                cl_wcfc,
                os.path.join(
                    workdir, out_paths["pmem"]["plots"], "footprint_cl" + str(idcl) + ".png"
                ),
            )
        ...
        if (
            bkg_wmask_cfc < pmem_cfg["global_conditions"]["bkg_cover_min"]
            or cl_wcfc < pmem_cfg["global_conditions"]["cl_cover_min"]
        ):
            data_richness["flag_pmem"][i] = 4
            continue
        ...
        if data_members is not None:
            if flag_init_members == 0:
                ...
                data_members_thread = np.hstack((data_members_thread, data_members))
...
def pmem_1cluster(
    ...
    r200_arcmin = 60.0 * np.degrees(r200 / my_cluster["conv_factor"])
    ...
    if verbose >= 1:
        ...
        print("    slope (rad-overd)  = " + str(round(my_cluster["slope_dprofile"], 2)))
    ...
    if verbose >= 3:
        plot_mag_counts(
            ...
            data_gal,
            galcat["keys"],
            pmem_cfg["radial_bin_specs"],
            pmem_cfg["mag_bin_specs"],
            data_fp_mask,
            footprint,
            os.path.join(
                workdir, out_paths["pmem"]["plots"], "mag_counts_cl" + my_cluster["idcl"] + ".png"
            ),
        )
    ...
    n200_pmem_raw = n200_raw_from_pmems(
        pmem, data_gal, galcat["keys"], my_cluster, pmem_cfg["richness_specs"], r200
    )

    n200_pmem_cor, n200_pmem_cor_err = ngals_cor_from_pmems(
        pmem_cfg, pmem, pmem_err, data_gal, galcat["keys"], r200, my_cluster, data_fp, footprint
    )
    n_pmem_500kpc, n_pmem_500kpc_err = ngals_cor_from_pmems(
        pmem_cfg, pmem, pmem_err, data_gal, galcat["keys"], 0.5, my_cluster, data_fp, footprint
    )
    ...
    print("    n200 (cor / raw) = ", np.round(n200_pmem_cor, 2), " / ", np.round(n200_pmem_raw, 2))
    ...
    # Feed richness table
    cond_pmem = (data_gal["dist2cl_mpc"] / r200 < pmem_cfg["pmem_specs"]["dmax"]) & (pmem > 0.0)
    ...
    if verbose >= 2:
        plot_richness_vec(
            rad_vec_mpc, rich_vec, richerr_vec, rfit, r200, my_cluster["idcl"], out_paths
        )

    # Prepare Pmem table : ngal entries / cluster
    cond_pmem = (data_gal["dist2cl_mpc"] / r200 < 1.5) & (pmem > 0.0)
    ...
    data_members["id_cl"] = np.full(len(data_gal[cond_pmem]), my_cluster["idcl"])
    data_members["zcl"] = np.full(len(data_gal[cond_pmem]), my_cluster["zcl"])
    ...
    data_members["zp-zcl_over_sig_dz"] = (
        data_gal[galcat["keys"]["key_zp"]][cond_pmem] - my_cluster["zcl"]
    ) / my_cluster["sig_dz"]
    data_members["mag"] = data_gal[galcat["keys"]["key_mag"]][cond_pmem]
    data_members["mag-mstar"] = data_gal[galcat["keys"]["key_mag"]][cond_pmem] - my_cluster["mstar"]
...
def tile_radius_pmem(admin, pmem_cfg, cosmo_params):
    ...
    if not admin["target_mode"]:
        ...
        tile_radius0 = (2.0 * hp.nside2pixarea(Nside_tile, degrees=True)) ** 0.5 / 2.0
...
def run_pmem_list(data_cls, config, dconfig, thread_id):
    ...
    print("Nr of clusters in thread ", len(data_cls[data_cls["thread_id"] == thread_id]))

    thread_dir = os.path.join(workdir, "threads", "thread_" + str(thread_id).zfill(3))
    ...
    if not os.path.isfile(os.path.join(thread_dir, out_paths["pmem"]["results"], "richness.fits")):
        ...
        t.write(
            os.path.join(thread_dir, out_paths["pmem"]["results"], "richness.fits"), overwrite=True
        )
        t = Table(data_members)  # , names=names)
        t.write(os.path.join(thread_dir, out_paths["pmem"]["results"], "pmem.fits"), overwrite=True)
...
def concatenate_calib_dz(all_tiles, pmem_cfg, workdir, calib_file):
    ...
    for it in range(0, len(all_tiles)):
        tile_dir = os.path.join(workdir, "tiles", "tile_" + str(int(all_tiles["id"][it])).zfill(5))
        if os.path.isfile(os.path.join(tile_dir, pmem_cfg["calib_dz"]["filename"])):
            list_calibs.append(os.path.join(tile_dir))
    # calib_file written on disc
    data_calib = concatenate_clusters(list_calibs, pmem_cfg["calib_dz"]["filename"], calib_file)
    return data_calib
...
def pmem_concatenate_tiles(all_tiles, out_paths, rich_file, pmem_file):
    ...
    for it in range(0, len(all_tiles)):
        tile_dir = os.path.join(
            out_paths["workdir"], "tiles", "tile_" + str(int(all_tiles["id"][it])).zfill(5)
        )
        list_clusters.append(os.path.join(tile_dir, out_paths["pmem"]["results"]))
        list_members.append(os.path.join(tile_dir, out_paths["pmem"]["results"]))
    data_richness = concatenate_clusters(list_clusters, "richness.fits", rich_file)
    data_members = concatenate_clusters(list_members, "pmem.fits", pmem_file)
    return data_richness, data_members
...
def pmem_concatenate_threads(all_tiles, out_paths, rich_file, pmem_file):
    ...
    list_members = []
    for it in range(0, len(all_tiles)):
        tile_dir = thread_dir_name(out_paths["workdir"], int(all_tiles["id"][it]))
        list_clusters.append(os.path.join(tile_dir, out_paths["pmem"]["results"]))
        list_members.append(os.path.join(tile_dir, out_paths["pmem"]["results"]))
    data_richness = concatenate_clusters(list_clusters, "richness.fits", rich_file)
    data_members = concatenate_clusters(list_members, "pmem.fits", pmem_file)
