...
def create_slurm_script(task, config, dconfig, narray, script):
    ...
    print("...task = ", task)
    print("      ntiles / max nthreads = ", narray, slurm_cfg["max_parallel"])
    ...
    if narray > 1:
        f.write(f"#SBATCH --cpus-per-task={slurm_cfg['cpus-per-task']}\n")
        f.write(f"#SBATCH --array=0-{narray - 1}%{slurm_cfg['max_parallel']}\n")
...
def make_tile_cat(tile_specs, sourcecat, footprint, maglim, wcfg):
    data_gal_tile = read_hpix_mosaicFitsCat(tile_specs["hpix_tile"], sourcecat["mosaic"]["dir"])
    data_gal_tile = data_gal_tile[data_gal_tile[sourcecat["keys"]["key_mag"]] <= np.float64(maglim)]

    data_fp_tile = read_hpix_mosaicFootprint(tile_specs["hpix_tile"], footprint["mosaic"]["dir"])
...
def read_sources_in_hpixs(dat, keys, hpix_arr, Nside, nest):
    hpix = hp.ang2pix(Nside, dat[keys["key_ra"]], dat[keys["key_dec"]], nest, lonlat=True)
...
def read_hpix_mosaicFitsCat(hpix_arr, datadir):
    i = 0
    for hh in hpix_arr:
        datas = read_FitsCat(os.path.join(datadir, str(hh) + ".fits"))
        if i == 0:
            data = np.copy(datas)
        else:
...
def read_hpix_mosaicFootprint(hpix_arr, datadir):
    i = 0
    for hh in hpix_arr:
        datas = read_FitsCat(os.path.join(datadir, str(hh) + "_footprint.fits"))
        if i == 0:
            data = np.copy(datas)
        else:
            data = np.append(data, datas)
...
def read_mosaicFitsCat_in_disc(galcat, tile, radius_deg):
    ...
    if len(relevant_fits_pixels) > 0:
        ...
        for i in range(0, len(relevant_fits_pixels)):
            dat_disc = read_FitsCat(os.path.join(gdir, str(relevant_fits_pixels[i]) + extension))
            dcen = np.degrees(
                dist_ang(
    ...
    if len(relevant_fits_pixels) > 0:
        ...
        for i in range(0, len(relevant_fits_pixels)):
            dat_disc = read_FitsCat(
                os.path.join(gdir, str(relevant_fits_pixels[i]) + "_footprint" + extension)
            )
            ra, dec = hp.pix2ang(
                footprint["Nside"], dat_disc[footprint["key_pixel"]], footprint["nest"], lonlat=True
            )
...
def read_mosaicFitsCat_in_hpix(galcat, hpix_tile, Nside_tile, nest_tile):
    ...
    ra_fits, dec_fits = hp.pix2ang(Nside_fits, hpix_fits, nest_fits, lonlat=True)
    hpix_fits_tile = hp.ang2pix(Nside_tile, ra_fits, dec_fits, nest_tile, lonlat=True)
    relevant_fits_pixels = np.unique(hpix_fits[np.isin(hpix_fits_tile, hpix_tile)])
    if len(relevant_fits_pixels) > 0:
        # merge intersecting fits
        for i in range(0, len(relevant_fits_pixels)):
            dat = read_FitsCat(os.path.join(gdir, str(relevant_fits_pixels[i]) + extension))
            if i == 0:
...
def create_mosaic_footprint(footprint, fpath):
    ...
    ra0, dec0 = hp.pix2ang(footprint["Nside"], hpix0, footprint["nest"], lonlat=True)
    hpix = hp.ang2pix(
        footprint["mosaic"]["Nside"], ra0, dec0, footprint["mosaic"]["nest"], lonlat=True
    )
    for hpu in np.unique(hpix):
        all_cols = fits.ColDefs(
            [
                fits.Column(
                    name=footprint["key_pixel"], format="K", array=hpix0[np.isin(hpix, hpu)]
                ),
                fits.Column(name="ra", format="D", array=ra0[np.isin(hpix, hpu)]),
                fits.Column(name="dec", format="D", array=dec0[np.isin(hpix, hpu)]),
                fits.Column(
                    name=footprint["key_frac"], format="E", array=frac0[np.isin(hpix, hpu)]
                ),
            ]
        )
        hdu = fits.BinTableHDU.from_columns(all_cols)
        hdu.writeto(os.path.join(fpath, str(hpu) + "_footprint.fits"), overwrite=True)
    return
...
def filter_hpx_tile(data, cat, tile_specs):
    ...
    Returns:
        _type_: _description_
    """
    ra, dec = data[cat["keys"]["key_ra"]], data[cat["keys"]["key_dec"]]
...
def area_ann_deg2(theta_1, theta_2):
    ...
    area = (
        2.0
        * np.pi
        * (np.cos(np.radians(theta_1)) - np.cos(np.radians(theta_2)))
        * (180.0 / np.pi) ** 2
    )
    return area
...
def all_hpx_in_annulus(ra, dec, radius_in_deg, radius_out_deg, hpx_meta, inclusive):
    ...
    if radius_in_deg > 0.0:
        ...
        id_annulus = np.isin(pixels_in_disc, pixels_in_disc_in, assume_unique=True, invert=True)
...
def hpx_in_annulus(ra, dec, radius_in_deg, radius_out_deg, data_fp, hpx_meta, inclusive):
    """
    ...
    """
    Nside, _nest = hpx_meta["Nside"], hpx_meta["nest"]
    hpix, frac = data_fp[hpx_meta["key_pixel"]], data_fp[hpx_meta["key_frac"]]

    area_pix = hp.nside2pixarea(Nside, degrees=True)
    pixels_in_ann = all_hpx_in_annulus(ra, dec, radius_in_deg, radius_out_deg, hpx_meta, inclusive)
...
def hpix_list_bary(hpix, Nside, nest):
    ...
    if (np.amax(ra) - np.amin(ra)) > 180.0:
        ra[ra > 180.0] = ra[ra > 180.0] - 360.0
        ra_bary = np.mean(ra)
        if ra_bary < 0.0:
            ra_bary = ra_bary + 360.0
    else:
        ra_bary = np.mean(ra)
...
def hpx_split_survey_equal_tiles(footprint_files, footprint, tiling):
    ...
    for i in range(0, len(hpix_unique)):
        ...
        dat_fp_hpix = read_mosaicFootprint_in_hpix(footprint, hpix_unique[i], Nside_tile, nest_tile)
...
def max_dist_to_parent_hpix(hpix_parent, hpix, Nside_tile, nest_tile, footprint, Nside_fp, nest_fp):
    dat_fp_hpix = read_mosaicFootprint_in_hpix(footprint, hpix, Nside_tile, nest_tile)
    ra, dec = hp.pix2ang(Nside_fp, dat_fp_hpix[footprint["key_pixel"]], nest_fp, lonlat=True)
    rac_parent, decc_parent = hp.pix2ang(Nside_tile, hpix_parent, nest_tile, lonlat=True)
    dist2parent_hpix = np.degrees(dist_ang(ra, dec, rac_parent, decc_parent))
...
def scan_survey_ra(hpix, Nside, nest):
    ...
    ramin, ramax = np.amin(ra), np.amax(ra)
    if ramin < size and 360.0 - ramax < size:
        ra0_crossing = True
        ...
        ra_split = (
            np.amax(ra[np.argwhere(labels == 0).T]) + np.amin(ra[np.argwhere(labels == 1).T])
        ) / 2.0
...
def tile_center(hplist, Nside, nest):
    ...
    ra, dec = hp.pix2ang(Nside, hplist, nest, lonlat=True)
    deccen = np.mean(dec)

    if ra0_crossing:
        ra[ra > ra_split] = ra[ra > ra_split] - 360.0
    racen = np.mean(ra)
    if racen < 0.0:
        racen += 360.0
    return racen, deccen
...
def tile_radius(hplist, Nside, nest):
    ...
    ra, dec = hp.pix2ang(Nside, hplist, nest, lonlat=True)
    dcen = np.degrees(dist_ang(ra, dec, racen, deccen))
    half_pix_size = 0.5 * 1.45 * hp.nside2pixarea(Nside, degrees=True) ** 0.5
...
def sky_partition(tiling, gdir, footprint, workdir):
    ...
    if os.path.isfile(os.path.join(workdir, tiling["rpath"], tiling["tiles_filename"])):
        ntiles = len(read_FitsCat(os.path.join(workdir, tiling["rpath"], tiling["tiles_filename"])))
    ...
    # partition
    if not os.path.isfile(os.path.join(workdir, tiling["rpath"], tiling["sky_partition_npy"])):
        # partitionning with KMeans algorithm and save hpix's in npy
        hp_ra0, hp_dec0 = hp.pix2ang(Nside, hpix_fits, nest, lonlat=True)
        if ra0_crossing:
            hp_ra0[hp_ra0 > ra_split] = hp_ra0[hp_ra0 > ra_split] - 360.0
        ...
        plt.scatter(hp_ra0, hp_dec0, c=labels, s=5, cmap=plt.cm.nipy_spectral)
        plt.xlabel("R.A. [deg]")
        plt.ylabel("Dec. [deg]")
        plt.title("Sky partinioning : " + str(ntiles) + " tiles")
        plt.savefig(os.path.join(workdir, tiling["rpath"], "partition.png"))

    # compute the tile overlaps and save lists of hpix's in npy
    if not os.path.isfile(os.path.join(workdir, tiling["rpath"], tiling["tiles_npy"])):
        # compute the number of layers for the overlap
        N_layers = 1 + int(overlap_deg / hp.nside2pixarea(Nside, degrees=True) ** 0.5)
        overlap_eff_size = N_layers * 1.42 * hp.nside2pixarea(Nside, degrees=True) ** 0.5
        ...
        for i in range(0, ntiles):
            ...
            for j in range(0, N_layers):
                ...
                all_hp = np.unique(
                    np.concatenate(hp.get_all_neighbours(Nside, hp_tile, nest=True)).ravel()
                )
                hp_neigh_in_survey = all_hp[np.isin(all_hp, hpix_fits)]
                hp_neigh = hp_neigh_in_survey[np.isin(hp_neigh_in_survey, hp_tile, invert=True)]
        ...
        np.save(
            os.path.join(workdir, tiling["rpath"], tiling["tiles_npy"]),
    ...
    tiles = np.load(os.path.join(workdir, tiling["rpath"], tiling["tiles_npy"]), allow_pickle=True)
    ntiles = len(partition)
    ...
    for i in range(0, ntiles):
        ...
        racen[i], deccen[i] = tile_center(hp_tile, Nside, nest)
        ...
        area_core[i] = area_core[i] * hp.nside2pixarea(footprint["Nside"], degrees=True)
        for hh in hp_tile:
            hpix, hfrac = read_FitsFootprint(
                os.path.join(mdir, str(hh) + "_footprint.fits"), footprint
            )
            area_tile[i] += np.sum(hfrac)
        area_tile[i] = area_tile[i] * hp.nside2pixarea(footprint["Nside"], degrees=True)
    ...
    data_tiles = np.zeros(
        ...
        dtype={
            ...
            "formats": ("i8", "i8", "i8", "f8", "f8", "f8", "f8", "f8"),
        },
    )
    ...
    t = Table(data_tiles)
    t.write(os.path.join(workdir, tiling["rpath"], tiling["tiles_filename"]), overwrite=True)
    return ntiles
...
def radec_window(racen, deccen, radius):
    ...
    decmin, decmax = deccen - radius, deccen + radius
...
def draw_hpixels(pixels, Nside, nest, ra0_crossing, ra360_crossing, color, label):
    ...
    for p in pixels[:]:
        if i == 0:
            plt.fill(
                *get_boundaries(p, Nside, nest, ra0_crossing, ra360_crossing),
                color=color,
                alpha=0.1,
                zorder=2,
                label=label,
...
def plot_tile(tile_id, hp_lab, all_hp_neigh, hpix_fits, Nside, nest, osize, outdir):
    create_directory(os.path.join(outdir, "tile_plots"))

    hpix_fits_out = hpix_fits[np.isin(hpix_fits, np.hstack((hp_lab, all_hp_neigh)), invert=True)]
    ra_core, dec_core = hp.pix2ang(Nside, hp_lab, nest, lonlat=True)

    racen, deccen = np.median(ra_core), np.median(dec_core)
    if np.std(ra_core) > 100.0:
        ra_core[ra_core >= 180.0] = ra_core[ra_core >= 180.0] - 360.0
        racen = np.median(ra_core)

    radius = 1.0 + osize + 1.42 * 0.5 * (len(hp_lab) * hp.nside2pixarea(Nside, degrees=True)) ** 0.5
    ramin, ramax, decmin, decmax = radec_window(racen, deccen, radius)
    ramin_ext, ramax_ext, decmin_ext, decmax_ext = radec_window(racen, deccen, radius + 1.0)
    ...
    plt.clf()
    fig, ax = plt.subplots()
    ax.set_aspect(1.0 / np.cos(np.radians(deccen)))

    # plot pixels of tile core
    draw_hpixels(hp_lab, Nside, nest, ra0_crossing, ra360_crossing, "r", "Core")
    # plot pixels of tile overlap
    draw_hpixels(all_hp_neigh, Nside, nest, ra0_crossing, ra360_crossing, "g", "Overlap")
    # plot pixels of surrounding survey pixels
    draw_hpixels(hpix_all, Nside, nest, ra0_crossing, ra360_crossing, "b", "Survey")
...
def create_tile_specs(target_mode, tiling, tile, hpix_core_list, hpix_tile_list):
    """_summary_
    ...
    area_deg2 = float(tile["npix_core"]) * hp.nside2pixarea(Nside, degrees=True)
...
def create_ptile_specs(target, radius, search_radius, radius_unit, dat_footprint, footprint):
    """_summary_
    ...
    coverfrac_30 = disc_coverfrac(target["ra"], target["dec"], 0.5, dat_footprint, footprint)
    coverfrac_5 = disc_coverfrac(target["ra"], target["dec"], 1.0 / 12.0, dat_footprint, footprint)
    area_deg2 = np.round(area_ann_deg2(0.0, radius), 3)
    eff_area_deg2 = disc_coverfrac(target["ra"], target["dec"], radius, dat_footprint, footprint)
    if radius_unit == "arcmin":
        radius_filter_deg = search_radius / 60.0
    if radius_unit == "mpc":
        radius_filter_deg = np.degrees((search_radius / target["conv_factor"]))
...
def cond_in_disc(rag, decg, hpxg, Nside, nest, racen, deccen, rad_deg):
    ...
    if pix_size > 0.1 * rad_deg:
        ...
        dist2cl[cond] = np.degrees(dist_ang(rag[cond], decg[cond], racen, deccen))
    else:
        ...
        pixels_edge = pixels_in_disc[
            np.isin(pixels_in_disc, pixels_in_disc_strict, invert=True, assume_unique=True)
        ]
        cond_strict = np.isin(hpxg, pixels_in_disc_strict)
        cond_edge = np.isin(hpxg, pixels_edge)

        dist2cl[cond_strict] = 0.0
        dist2cl[cond_edge] = np.degrees(dist_ang(rag[cond_edge], decg[cond_edge], racen, deccen))
...
def compute_gaussian_kernel_1d(kernel):
    ...
    for n in range(0, 3 * kernel + 1):
        x1 = mean - 1.0 / 2.0 + float(n)
        x2 = mean + 1.0 / 2.0 + float(n)
        res, err = quad(normal_distribution_function, x1, x2)
        kk = np.append(kk, 100.0 * res)
...
def concatenate_members(all_tiles, list_path_members, infilename, data_clusters, members_outfile):
    ...
    for it in range(0, len(all_tiles)):
        ...
        members = read_FitsCat(os.path.join(list_path_members[it], infilename))
        members_kept = members[np.isin(members["index_cl_tile"], clusters_id_in_tile)]
        ...
        clusters_tile[np.argsort(clusters_id_in_tile)]["index_cl_tile"]
...
def tiles_with_clusters(out_paths, all_tiles, code):
    ...
    for it in range(0, len(all_tiles)):
        ...
        if os.path.isfile(os.path.join(tile_dir, out_paths[code]["results"], "tile_info.fits")):
            if (
                read_FitsCat(os.path.join(tile_dir, out_paths[code]["results"], "tile_info.fits"))[
                    0
                ]["Nclusters"]
                > 0
            ):
...
def concatenate_cl_tiles(out_paths, all_tiles, code):
    ...
    for it in range(0, len(eff_tiles)):
        tile_dir = os.path.join(workdir, "tiles", "tile_" + str(int(eff_tiles["id"][it])).zfill(5))
        list_clusters.append(os.path.join(tile_dir, out_paths[code]["results"]))
    data_clusters0 = concatenate_clusters(
        list_clusters, "clusters.fits", os.path.join(workdir, "tmp", "clusters0.fits")
    )
...
def get_footprint(input_data_structure, footprint, workdir):
    if input_data_structure["footprint_hpx_mosaic"]:
        survey_footprint = os.path.join(workdir, "footprint", "survey_footprint.fits")
        if not os.path.isfile(survey_footprint):
            create_survey_footprint_from_mosaic(footprint, survey_footprint)
    else:
...
def update_data_structure(param_cfg, param_data):
    ...
    if not input_data_structure[survey]["footprint_hpx_mosaic"]:
        create_mosaic_footprint(footprint[survey], os.path.join(workdir, "footprint_mosaic"))
        param_data["footprint"][survey]["mosaic"]["dir"] = os.path.join(workdir, "footprint_mosaic")
