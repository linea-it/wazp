...
from wazp.utils import sky_partition
...
# compute zp slicing
compute_zpslices(param_data["zp_metrics"][survey][ref_filter], detection_cfg, -1.0, workdir)
...
job_id1 = slurm_submit("detection_tile", config, dconfig, narray=ntiles_detection)
# concatenate
job_id2 = slurm_submit("detection_concatenate", config, dconfig, dep=job_id1)

# run Pmem
print("Run pmem_tile / slurm ")
job_id3 = slurm_submit("pmem_tile", config, dconfig, narray=ntiles_pmem, dep=job_id2)

# concatenate Pmems
job_id4 = slurm_submit("pmem_concatenate", config, dconfig, dep=job_id3)
