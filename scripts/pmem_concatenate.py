...
from wazp.pmem import concatenate_calib_dz
...
tiles_filename = os.path.join(
    workdir, admin["tiling_pmem"]["rpath"], admin["tiling_pmem"]["tiles_filename"]
)
...
if pmem_cfg["calib_dz"]["mode"]:
    data_calib = concatenate_calib_dz(
        ...
        pmem_cfg,
        workdir,
        os.path.join(workdir, "calib", pmem_cfg["calib_dz"]["filename"]),
    )
...
official_wazp_cat(
    data_clusters_with_rich,
    param_cfg["clcat"][clusters]["keys"],
    pmem_cfg["richness_specs"],
    detection_cfg["rich_min"],
